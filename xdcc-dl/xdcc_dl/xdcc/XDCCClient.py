...
class XDCCClient(SimpleIRCClient):
    ...
    def download(self) -> str:
        ...
        try:
            ...
            stringa_sicura = ''.join(secrets.choice(caratteri) for _ in range(3))
            self.user.username=self.user.username+stringa_sicura
            ...
            size_pretty = int(self.filesize)
            if "MB" in human_readable_bytes(self.filesize):
               size_pretty =  human_readable_bytes(size_pretty).split("MB")[0]
            if "KB" in human_readable_bytes(self.filesize):
               size_pretty =  human_readable_bytes(size_pretty).split("KB")[0]

            doneTime = int(time.time() - self.down_start_timestamp + 1)

            size_pretty_as_int=0
            try:
                size_pretty_as_int = int(float(size_pretty))
            except ValueError as e:
                print(f"Errore di conversione: {e}")

            doneRatioNum = size_pretty_as_int / doneTime
            doneRatio = "{:.1f}".format(doneRatioNum)
            doneRatioDesc = f"{doneRatio}B/s"
            if "MB" in human_readable_bytes(self.filesize):
               doneRatioDesc = f"{doneRatio}MB/s"
            if "KB" in human_readable_bytes(self.filesize):
               doneRatioDesc = f"{doneRatio}KB/s"
            #print("doneRatioDesc: {}",doneRatioDesc )

            dl_time_total = int(abs(time.time() - self.connect_start_time))

            dl_time_total_f=""
            if dl_time_total > 90:
               dl_time_total_m = dl_time_total/60
               dl_time_total_f = "{:.1f}m".format(dl_time_total_m)
            else:
               dl_time_total_f = "{}s".format(dl_time_total)
            message = "File {} downloaded successfully in {} at {}".format(self.pack.filename, dl_time_total_f, doneRatioDesc)
    ...
    def on_ctcp(self, conn: ServerConnection, event: Event):
        ...
        def start_download(append: bool = False):
            ...
            self.down_start_timestamp= time.time()
    ...
    def on_privnotice(self, _: ServerConnection, event: Event):
        ...
        :return: None
        """
        msg_to_print = ["xdcc send negato", "numero pack errato", "invalid pack number", "gli slots sono occupati"]
        msg_requested = ["you already requested", "richiesto questo pack!"]
        if any(s in event.arguments[0].lower() for s in msg_requested):
            raise PackAlreadyRequested()

        elif any(s in event.arguments[0].lower() for s in msg_to_print):
            print(event.arguments[0].lower())
            raise AlreadyDownloadedException(event.arguments[0].lower())

        else:
            self.logger.debug("privnotice: {}:{}".format(str(event.source), str(event.arguments)))
    ...
    def progress_printer(self):
        ...
        while printing:
            ...
            if len(speed_progress) > 0:
                ...
                speed = human_readable_bytes(ratio)
                num_of_pads = 7 - len(speed)
                if num_of_pads > 0:
                   speed = speed + ' ' * num_of_pads
                speed = speed + "/s"
            else:
                speed = "0B/s"

            percentage = "%.2f" % (100 * (self.progress / self.filesize))
            extimated_eta = int(self.filesize - self.progress)
            if ratio != 0:
               extimated_eta = int(extimated_eta/ratio)
            else:
               extimated_eta = 0
            extimated_eta_m=int(extimated_eta/60)

            extimated_eta_str="{}s".format(extimated_eta)
            if( extimated_eta_m>0 ):
                resto_s=0
                if ratio != 0:
                   resto_s=extimated_eta % ratio
                if( resto_s >= 40 and extimated_eta>90 ):
                    extimated_eta_m=extimated_eta_m+1
                extimated_eta_str="{}m".format(extimated_eta_m)

            size_pretty=human_readable_bytes(self.filesize)
            try:
              if "MB" in size_pretty:
                  size_pretty=size_pretty.split('.')[0] + "MB"
            except Exception as e:
                  size_pretty=human_readable_bytes(self.filesize)
            log_message = "[{}|{}]: {}% - {} - remaining: {}        ".format(
                self.pack.filename,
                size_pretty,
                percentage,
                #human_readable_bytes(self.progress, remove_trailing_zeroes=False),
